...
import { FoodBlock, FoodPage } from "@/__generated__/graphql";
  ...
  return (
    <main>
      <div className="bg-white">
        ...
        {
          data.FoodPage.items.map((item: FoodPage) => (
                ...
                {item.FoodArea?.map((food) => (
                  <div key={food?.ContentLink?.Expanded?.Name} className="group relative">
                    <div className="aspect-h-1 aspect-w-1 w-full overflow-hidden rounded-md bg-gray-200 lg:aspect-none group-hover:opacity-75 lg:h-80">
                      <img
                         src={`https://app-ocxcfoodindw21vqprod.cms.optimizely.com${(food?.ContentLink?.Expanded as FoodBlock).Image}` || ""}
                    ...
                    <div className="mt-4 flex justify-between">
                      <div>
                        <h3 className="text-sm text-gray-700">
                          <a href={food?.ContentLink?.Url || ""}>
                            <span aria-hidden="true" className="absolute inset-0" />
                            {food?.ContentLink?.Expanded?.Url || ""}
                          </a>
                        </h3>
                        <p className="mt-1 text-sm text-gray-500">{(food?.ContentLink?.Expanded as FoodBlock).Types || ""}</p>
                      </div>
