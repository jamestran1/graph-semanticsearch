...
export default function HomeComponent({ locale }: HomeComponentProps) {
  ...
  const {data, error}  = useSuspenseQuery(HomeQuery, {variables: { locale: [`${locale}`] }});
  if (error) return `Error! ${error.message}`

  const handleSearch = (event: any) => {
    if (event.key === "Enter") {
      router.push(`/${locale}/search?q=${event.target.value}`)
    }
  };
              ...
              {item.FoodArea?.map((food) => (
                <div key={food?.ContentLink?.Expanded?.Name} className="group relative">
                  <div className="aspect-h-1 aspect-w-1 w-full overflow-hidden rounded-md bg-gray-200 lg:aspect-none group-hover:opacity-75 lg:h-80">
                    <img
                      src={`${process.env.CMS_SAAS}${(food?.ContentLink?.Expanded as FoodPage).Image}` || ""}
